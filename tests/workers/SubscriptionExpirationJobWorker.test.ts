...
jest.mock('../../src/config/env', () => ({
   config: {
      ...
      SUBSCRIPTION_EXPIRATION_CRON: '2 0 * * *',
   },
}));
...
import { SubscriptionExpirationJobWorker } from '../../src/workers/SubscriptionExpirationJobWorker';
import { redisService } from '../../src/services/redis';
import { UserSubscriptionService } from '../../src/services/UserSubscriptionService';

describe('SubscriptionExpirationJobWorker', () => {
   const mockPrisma = {} as PrismaClient;
   let worker: SubscriptionExpirationJobWorker;
   let mockExpire: jest.Mock;

   beforeEach(() => {
      jest.clearAllMocks();
      mockExpire = jest.fn().mockResolvedValue({ processed: 1, failed: 0, errors: [] });
      (UserSubscriptionService as jest.Mock).mockImplementation(() => ({
         expireDueCanceledSubscriptions: mockExpire,
      }));
      worker = new SubscriptionExpirationJobWorker(mockPrisma);
   });

   it('runs expireDueCanceledSubscriptions when lock is acquired', async () => {
      (redisService.acquireLock as jest.Mock).mockResolvedValue(true);

      await worker.runJob();

      expect(mockExpire).toHaveBeenCalledTimes(1);
      expect(redisService.releaseLock).toHaveBeenCalledWith('subscription-expiration-job');
   });
   ...
   it('does not schedule when SUBSCRIPTION_JOBS_ENABLED is false', async () => {
      ...
      const disabledWorker = new SubscriptionExpirationJobWorker(mockPrisma);
